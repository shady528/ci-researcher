...
Routing logic:
  ...
  source_mode = "internal" → ChromaDB only, session-scoped
...
def _rag_search(query: str, session_id: str) -> list[SourceDoc]:
    print(f"    📁 RAG search: {query[:55]}…")
    results = query_internal_docs(query, session_id=session_id, top_k=5)
    if results:
...
def researcher_node(state: AgentState) -> dict:
    ...
    # Query selection — gap-fill uses gap queries for both web and RAG
    if iteration > 0 and state.get("gaps"):
        unresolved = [g for g in state["gaps"] if not g["resolved"]]
        if unresolved:
            gap_queries = [g["suggested_query"] for g in unresolved]
            web_queries = gap_queries
            rag_queries = gap_queries
            print(f"\n[RESEARCHER] 🔍 Gap-fill iteration {iteration} | "
                  f"{len(gap_queries)} targeted queries")
        else:
            web_queries = state["queries"]
            rag_queries = state.get("rag_queries", [])
            print(f"\n[RESEARCHER] 🔍 Iteration {iteration} | No unresolved gaps")
    else:
        web_queries = state["queries"]
        rag_queries = state.get("rag_queries", [])
        mode_label  = {
    ...
    # RAG search — always session-scoped
    if source_mode in ("internal", "hybrid"):
        if not session_id:
            print(f"    ⚠️  No session_id — skipping RAG search")
        else:
            queries_to_run = rag_queries if rag_queries else web_queries
            for query in queries_to_run:
                new_docs.extend(_rag_search(query, session_id))
    ...
    print(f"  → {len(unique)} new unique docs ({web_count} web, {internal_count} internal)")
